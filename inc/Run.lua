...
av = Token:match("(%d+)")
redis:set(av..":VERSION",1)
redis:set(av..":SUDO_ID:",SUDO_USER)
redis:set(av..":DataCenter:",'German')
redis:set(av..":UserNameBot:",BOT_User)
redis:set(av..":NameBot:",BOT_NAME)
redis:hset(av..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3av_INSTALL","Yes")
info = {} 
...
Bot_User = redis:get(av..":UserNameBot:")
SUDO_ID = tonumber(redis:get(av..":SUDO_ID:"))
SUDO_USER = redis:hgetall(av..'username:'..SUDO_ID).username
version = redis:get(av..":VERSION")
DataCenter = redis:get(av..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..av..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(av..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(av..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(av..'group:add'..msg.chat_id_) then 
...
msg.TheRank =  ' مالكه السورس '
msg.Rank = 1
elseif msg.sender_user_id_ == SUDO_ID then 
msg.TheRankCmd = 'المطور' 
msg.TheRank = 'مطور اساسي' 
msg.Rank = 1
elseif redis:sismember(av..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(av..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(av..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(av..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(av..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(av..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(av..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(av..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(av..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(av..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(av..':WELCOME_BOT'),[[• مرحباً انا بوت ]]..redis:get(av..':NameBot:')..[[ .
• اختـصاصـي حمايـة الـمجموعات
• تأكد من رفعـك للبـوت مشـرف في مجموعـتك واكتب تفعـيل 
قناة سورس ايـف 🇪🇬@Av_bot1
...
if msg.GroupActive and redis:get(av..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(av..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(av..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(av..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.av then 
if msg.text and ScriptFile.iav then
for k, av in pairs(ScriptFile.av) do
local SearchText = msg.text:match(av)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.iav(msg,{msg.text:match(av)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",av.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dav then
if ScriptFile.dav(msg) == false then
return false
end
...
if Plug.av then 
if msg.text and Plug.iav then
for k, av in pairs(Plug.av) do
local SearchText = msg.text:match(av)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(av..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(av..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.iav(msg,{msg.text:match(av)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",av.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source av")
end 

end
else
print("The File Script.lua Not Runing in The Source av")
...
function tdcli_update_callback(data)
	...
	redis:srem(av..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/mohamed2512000/SouRceav/main/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/mohamed2512000/SouRceav/main/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/mohamed2512000/SouRceav/main/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/mohamed2512000/SouRceav/main/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/mohamed2512000/SouRceav/main/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/mohamed2512000/SouRceav/main/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(av..":SUDO_ID:")
redis:hset(av..'username:'..tonumber(mmd),'username',msg.text)
...
download(stk,32)
	...
	if redis:get(av..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(av..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(av..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(av..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(av..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(av..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(av..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(av..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(av..'group:add'..data.chat_.id_) then
	redis:set(av..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(av..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(av..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(av..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	av..'group:name'..k,IDS.Title,
	av..'num_msg_av'..k,5,
	av..'group:add'..k,true,
	av..'lock_link'..k,true,
	av..'lock_spam'..k,true,
	av..'lock_webpage'..k,true,
	av..'lock_markdown'..k,true,
	av..'lock_flood'..k,true,
	av..'lock_bots'..k,true,
	av..'mute_forward'..k,true,
	av..'mute_contact'..k,true,
	av..'mute_document'..k,true,
	av..'mute_inline'..k,true,
	av..'lock_username'..k,true,
	av..'replay'..k,true
	)
	redis:sadd(av..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(av..'username:'..ID,'username',user)
	redis:sadd(av..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(av..'username:'..ID,'username',user)
	redis:sadd(av..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(av..'username:'..ID,'username',user)
	redis:sadd(av..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'• ** تم رفع النسخه‏‏ الاحتياطيه\n• ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(av..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(av..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(av..'username:'..data.user_.id_).username
	...
	redis:hset(av..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
